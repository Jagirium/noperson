...
///
/// Returns a 3×3 homogeneous transformation matrix.
///
...
pub fn similarity_transform(src: &[[f32; 2]], dst: &[[f32; 2]]) -> Matrix3<f64> {
    let n = src.len();
...
/// Extract 2×3 affine coefficients from a 3×3 homogeneous matrix.
/// Format: [[a00, a01, a02], [a10, a11, a12]] — compatible with NPP.
pub fn matrix3_to_2x3(m: &Matrix3<f64>) -> [[f64; 3]; 2] {
    [
        [m[(0, 0)], m[(0, 1)], m[(0, 2)]],
        [m[(1, 0)], m[(1, 1)], m[(1, 2)]],
    ]
...
pub fn estimate_face_affine(
    ...
    let m = similarity_transform(landmarks_5, template);
    matrix3_to_2x3(&m)
}
...
mod tests {
    ...
    #[test]
    fn test_invert_2x3_roundtrip() {
        let m = [[2.0, 0.5, 10.0], [0.3, 1.5, 20.0]];
        let inv = invert_2x3(&m);
        // Apply m then inv should give identity
        let a = m[0][0] * inv[0][0] + m[0][1] * inv[1][0];
        let b = m[0][0] * inv[0][1] + m[0][1] * inv[1][1];
