...
// fixed chunks; subgroup ballots retain exact model index order on wave32 and
// wave64 hardware.

__device__ __forceinline__ unsigned int ordered_chunk_offset(
    const unsigned int selected,
    unsigned int* subgroup_counts,
    unsigned int* subgroup_offsets,
    ...
    const unsigned long long selected_mask = noperson_subgroup_ballot(selected != 0u);
    if (lane + 1u == warpSize || threadIdx.x + 1u == blockDim.x) {
        subgroup_counts[subgroup] = noperson_popcount(selected_mask);
    }
    __syncthreads();

    if (subgroup == 0u && lane < subgroup_count) {
        unsigned int offset = 0;
        for (unsigned int index = 0; index < lane; ++index) {
            offset += subgroup_counts[index];
        }
        subgroup_offsets[lane] = offset;
        if (lane + 1u == subgroup_count) {
            *chunk_count = offset + subgroup_counts[lane];
        }
    }
    __syncthreads();

    return subgroup_offsets[subgroup]
        + noperson_popcount(selected_mask & noperson_lower_lane_mask(lane));
}
...
void yolo_face_compact_kernel(
    ...
    __shared__ unsigned int subgroup_counts[NOPERSON_MAX_SUBGROUPS_PER_BLOCK];
    __shared__ unsigned int subgroup_offsets[NOPERSON_MAX_SUBGROUPS_PER_BLOCK];
    ...
    for (unsigned int base = 0; base < anchors; base += 256) {
        ...
        const unsigned int exclusive =
            ordered_chunk_offset(selected, subgroup_counts, subgroup_offsets, &chunk_count);
